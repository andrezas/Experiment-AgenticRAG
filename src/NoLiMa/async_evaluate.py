...
from resources.data.book_haystack import BookHaystack
...
class NoLiMa_Tester:
    def __init__(
        ...
        if not pathlib.Path(self.model_config_path).exists():
            raise FileNotFoundError(f"Model configuration for {model_name} not found in {model_configs_dir}")

        with pathlib.Path(self.model_config_path).open() as file:
        ...
        if not pathlib.Path(haystack_path).exists():
    ...
    def _evaluate_response(self, response: str, gold_answers=None) -> int:
        ...
        if self.metric == "lastline_EM":
            return int(response.strip().split("\n")[-1] in gold_answers)
        if self.metric == "lastline_contains":
    ...
    def evaluate(self) -> None:
        ...
        outputs = {
            ...
            "model_name": f"{self.model_name}_FAKE",
        ...
        if pathlib.Path(results_path).exists():
            print(f"Fake results already exist at {results_path}")
            return

        if self.prevent_duplicate:
            for result_filename in os.listdir(self.results_dir):
                with pathlib.Path(os.path.join(self.results_dir, result_filename)).open() as file:
                    other_result = json.load(file)
                if outputs["test_hash"] == self.get_hash(other_result):
                    print("Duplicate test found -- TEST_HASH:", outputs["test_hash"])
                    return

        for i in tqdm(
            np.linspace(
                self.document_depth_percent_min, self.document_depth_percent_max, self.document_depth_percent_intervals
            )
        ):
            needle_depth = i / 100
            api_output = {}
            ...
            if "{CHAR}" in self.needle:
                ...
                current_gold = [selected_character]
            else:
                needle = self.needle
                current_gold = self.gold_answers
            ...
            api_output["question"] = retrieval_question
            api_output["generated_context"] = placement_output["text"]  # <-- O contexto gerado (haystack)
            api_output["generated_prompt"] = filled_template  # <-- O prompt final estruturado
            api_output["placement_metadata"] = {k: v for k, v in placement_output.items() if k != "text"}

            outputs["results"].append(api_output)

        # Grava o JSON completo
        with pathlib.Path(results_path).open("w") as file:
            json.dump(outputs, file, indent=4)

        print(f"Fake results with prompts and contexts saved at {results_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--document_depth_percent_intervals", type=int, default=35, help="Number of intervals between min and max depth"
    )
    ...
    parser.add_argument(
        "--log_placements_dir", type=str, default="", help="Directory to save needle placements for debugging"
    )
