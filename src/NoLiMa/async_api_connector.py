...
class APIConnector:
    def __init__(self, api_key: str, api_url: str, api_provider: str, model: str, **kwargs) -> None:
        """
        ...
        elif api_provider in DEFAULT_TOKENIZER_MAPPING:
        ...
        if api_provider == "openai":
            ...
            self.api = AsyncAzureOpenAI(
                api_key=api_key, azure_endpoint=api_url, api_version=kwargs["azure_api_version"]
            )
        elif api_provider == "vllm":
            self.api = AsyncOpenAI(
                api_key=api_key, base_url=api_url, max_retries=kwargs["max_retries"], timeout=kwargs["timeout"]
            )
    ...
    def decode(self, tokens: list) -> str:
        ...
        """
        if self.tokenizer_type == "huggingface" or self.tokenizer_type == "tiktoken":
        ...
        if self.api_provider == "gemini":
            ...
            @retry(
                ...
                wait=wait_random_exponential(
                    multiplier=self.gemini_retry_config["multiplier"], max=self.gemini_retry_config["maximum"]
                ),
                stop=stop_after_delay(self.gemini_retry_config["timeout"]),
                retry=retry_if_exception_type((ClientError, httpx.ConnectTimeout, httpx.TimeoutException)),
            )
            async def generate_content():
