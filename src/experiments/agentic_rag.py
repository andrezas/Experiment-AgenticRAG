...
class AgenticRAG:
    ...
    async def run(
            ...
            for hit in search_result:
                if str(hit.id) not in seen_chunk_ids:
                    ...
                    retrieved_chunks_refs.append(
                        {"chunk_id": str(hit.id), "score": hit.score, "metadata": hit.payload.get("metadata", {})}
                    )
            if not context_texts:
                return (
                    "Nenhuma nova informação relevante foi encontrada com estes termos.Tente palavras-chave diferentes."
                )
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    (
                        "You are an investigative research agent. Your goal is to answer the user's question.\n"
                        ...
                        "Once you are certain of the answer based on your searches, "
        ...
        agent_executor = AgentExecutor(
            agent=agent, tools=tools, verbose=False, max_iterations=max_iterations, return_intermediate_steps=True
        )
